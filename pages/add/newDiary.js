...
const ROW_CHARS = Math.floor((RESOLUTION - 2 * MARGIN) / input.input.charWidth);
    ...
    onLoad: function(options) {
        ...
        if (options) {
            ...
            if (title) {
                this.setData({
                    'diary.title': title,
                    'diary.meta': util.formatDate(new Date()),
                    'diary.create_time': util.formatTime(new Date()),
                    'diary.cover': 'https://www.chenqiaodonghu.com/honghongDiary_images/background/blueSky.png'
                });
            }
        }
        app.getUserInfo(info => {
            that.setData({
                'diary.avatar': info.avatarUrl,
    ...
    getPoi() {
        ...
        wx.getLocation({
            ...
            success: function(res) {
                geo.mapRequest(
                    'geocoder', { 'location': geo.formatLocation(res) },
                    loc => {
                        let poi = {
